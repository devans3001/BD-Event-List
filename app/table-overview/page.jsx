...
import { Users, UserCheck, UserX, Table, Percent, Calendar, Home } from "lucide-react";
...
//   console.log(tableStats)
  ...
  const overallStats = {
    ...
    arrivedGuests: tableStats.reduce((sum, table) => sum + table.arrivedGuests, 0),
    completionRate: ((tableStats.reduce((sum, table) => sum + table.arrivedGuests, 0) / 
                     tableStats.reduce((sum, table) => sum + table.totalGuests, 0)) * 100) || 0
  };
  ...
  const getStatusColor = (table) => {
    if (table.arrivedGuests === table.totalGuests) return "border-green-200 bg-green-50";
    ...
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="text-center mb-8">
        <div className="flex items-center justify-center gap-3 mb-4">
          ...
          <h1 className="text-4xl font-bold text-indigo-600">Table Overview</h1>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-indigo-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-800">{overallStats.totalTables}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-blue-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-800">{overallStats.totalGuests}</p>
        ...
        <div className="bg-white p-6 rounded-xl shadow-md border-l-4 border-green-500">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-800">{overallStats.arrivedGuests}</p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {sortedTableStats.map((table) => {
          const completionPercentage = (table.arrivedGuests / table.totalGuests) * 100;
          
          return (
            <div 
              key={table.tableNum} 
              className={`p-6 rounded-xl shadow-md border-2 transition-all duration-200 hover:shadow-lg ${getStatusColor(table)}`}
            >
              {/* Table Header */}
              <div className="flex items-center justify-between mb-4">
                ...
                <span className={`px-2 py-1 rounded-full text-xs font-medium ${getCompletionColor(completionPercentage)}`}>
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="font-semibold text-green-600">{table.arrivedGuests}</span>
                </div>
      ...
      {tableStats.find(t => t.tableNum === 'No Table') && (
        <div className="mt-8 bg-yellow-50 border border-yellow-200 rounded-xl p-6">
          ...
          <p className="text-yellow-700">
            {tableStats.find(t => t.tableNum === 'No Table')?.guestCount} guests 
            ({tableStats.find(t => t.tableNum === 'No Table')?.totalGuests} people) 
            haven't been assigned to a table yet.
          </p>
        </div>
      )}
      ...
      {tableStats.length === 0 && (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold text-gray-600 mb-2">No Table Assignments</h3>
          <p className="text-gray-500">Guests haven't been assigned to tables yet.</p>
        </div>
      )}
    </div>
