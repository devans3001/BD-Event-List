...
export default function GuestItem({ guest }) {
  ...
  return (
    <div className={`p-6 rounded-lg border-2 transition-all duration-200 shadow-sm ${
      guest.arrived 
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          className={`px-6 py-1.5 rounded-lg font-medium transition-colors  ${
            guest.arrived
              ? 'bg-green-500 hover:bg-green-600 text-white'
              : 'bg-gray-100 hover:bg-gray-200 text-gray-800 border border-gray-300'
          } disabled:opacity-50 disabled:cursor-not-allowed`}
        >
          {isPending ? <ClipLoader size={16}/> : guest.arrived ? 'Arrived ✓' : 'Mark Arrived'}
        </button>
      </div>
      ...
      {guest.arrived && (
        <div className="mt-3 pt-3 border-t border-green-200">
          <div className="text-green-600 text-sm font-medium flex items-center gap-2">
            ✅ Checked in at {new Date(guest.updated_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
          </div>
        </div>
      )}
    </div>
